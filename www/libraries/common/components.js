...
prx.commonproperties.actions = { caption: 'Interactions', name: 'actions', type: 'action', value: function(item,name) { if (typeof(item.actions) == "undefined") { item.actions = []; } return item.actions.length; }, changeProperty: { caption: 'Interactions', selector: '', property: 'action', transitionable: false, changable: false } };
	...
	,onDisplay: function(item,containerid) {
		...
		if (_symbolindex > -1) {
			...
			if (item.scroll!='') {
				...
				if (item.scroll=='vertical' || item.scroll=='omni') {
					if(typeof(prx.symbols[_symbolindex].dimensions) != "undefined") {
						...
						_symbol_height = prx.symbols[_symbolindex].states[0].dimensions[1]+'px';
					}
				}	
			}
			...
			if(typeof(prx.symbols[_symbolindex].states) == "undefined") {
				...
				var _bg = prx.symbols[_symbolindex].states[0].background;
				var dataState = 'data-state-id="'+prx.symbols[_symbolindex].states[0].id+'"';
			}
